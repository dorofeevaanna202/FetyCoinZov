...
def start(message):
    ...
    btn5 = types.KeyboardButton("💎 Обменять")
...
def play_roulette(message):
    ...
    if rand < 0.005:
        ...
        red_numbers = [1, 3, 5, 7, 11, 13, 15]
        result_num = random.choice(red_numbers)
    else:
        black_numbers = [2, 4, 6, 8, 10, 12, 14]
        result_num = random.choice(black_numbers)
...
def transfer_hz(message):
    ...
    bot.send_message(
        message.chat.id,
        f"✅ Переведено {amount:,} Hz⭐ игроку @{target['username']}\n"
        f"Твой баланс: {player['balance']:,} Hz⭐".replace(',', '.')
    )
...
def transfer_gtz(message):
    ...
    bot.send_message(
        message.chat.id,
        f"✅ Переведено {amount:,} GtZ💎 игроку @{target['username']}\n"
        f"Твой баланс GtZ: {player['gtz']:,} 💎".replace(',', '.')
    )
...
def admin_give_hz(message):
    ...
    bot.send_message(
        message.chat.id,
        f"✅ Выдано {amount:,} Hz⭐ игроку @{target['username']}\n"
        f"Его баланс: {target['balance']:,} Hz⭐".replace(',', '.')
    )
...
def admin_give_gtz(message):
    ...
    bot.send_message(
        message.chat.id,
        f"✅ Выдано {amount:,} GtZ💎 игроку @{target['username']}\n"
        f"Его баланс GtZ: {target['gtz']:,} 💎".replace(',', '.')
    )
